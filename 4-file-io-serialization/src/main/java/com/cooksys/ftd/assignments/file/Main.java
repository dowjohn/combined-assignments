...
public class Main {
	public static final String STUDENT_DIRECTORY_STRING = "input/memphis/08-08-2016/students/";
	public static final String INSTRUCTOR_STRING = "input/memphis/08-08-2016/instructor.xml";
	public static final String SESSION_STRING = "output/session.xml";
	...
	public static void writeSession(Session session, File sessionFile, JAXBContext jaxb) {
        ...
        try {
            ...
            marshaller.marshal(new File(SESSION_STRING), System.out);
        } catch (JAXBException e) {
            e.printStackTrace();
        }
	}
	...
	public static void main(String[] args) throws JAXBException {
		JAXBContext contactContext = JAXBContext.newInstance(Contact.class, Student.class, Session.class);
		File studentDirectory = new File(STUDENT_DIRECTORY_STRING);
//
//		try {
//			ArrayList<Student> students = new ArrayList<>();
//			students.addAll(readStudents(studentDirectory, contactContext));
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
        Session session = new Session();
        session.setInstructor(readInstructor(new File(INSTRUCTOR_STRING), contactContext));
        session.setStudents(readStudents(new File(STUDENT_DIRECTORY_STRING), contactContext));
        session.setLocation("Memphis");
        session.setStartDate("1-1-12");
        writeSession(session, new File(SESSION_STRING), contactContext);
    }
}
